...
_RULES = (
    ...
    _Rule(SafetyCategory.DAMAGED_CABLES, RiskLevel.IMMEDIATE_STOP, re.compile(r"\bcav[oi]\s+(?:elettric[oi]\s+)?(?:danneggiat[oi]|spellat[oi]|rovinat[oi])\b"), "Cavi danneggiati dichiarati."),
    _Rule(SafetyCategory.OVERHEATED_COMPONENTS, RiskLevel.WARNING, re.compile(r"\b(?:componente|scheda|motore|compressore)\s+(?:molto\s+)?(?:surriscaldat[oa]|bollente)\b"), "Possibile surriscaldamento dichiarato."),
